    ...
    <div>
      ...
      {Object.values(grouped).map((group,gi)=>(
        <div key={gi} style={{marginBottom:12}}>
          ...
          {group.alerts.map((a,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"var(--s2)",border:`1px solid ${aC[a.t]}22`,borderRadius:7,padding:"9px 12px",marginBottom:4}}><div style={{display:"flex",gap:8,alignItems:"flex-start"}}><div style={{width:7,height:7,borderRadius:"50%",background:aC[a.t],flexShrink:0,marginTop:3}}/><div><div style={{fontSize:11,color:aC[a.t],fontWeight:600,marginBottom:2,textTransform:"uppercase",letterSpacing:"0.03em"}}>{a.t}</div><div style={{fontSize:12,color:"var(--txt)"}}>{a.msg}</div></div></div></div>)}
        </div>
      ))}
