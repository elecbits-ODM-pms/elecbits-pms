      ...
      <div style={{flex:1,overflow:"auto"}}>
        {(view==="projects")&&<>
          ...
          <div style={{padding:"20px 32px",display:"grid",gridTemplateColumns:"repeat(3, 1fr)",gap:16}}>
            ...
            <div className="dash-stat-card">
              <div style={{fontSize:32,fontWeight:800,color:"#d97706",lineHeight:1}}>{(subView==="mine"?myProjects:projects).filter(p=>p.rag!=="green").length}</div>
            ...
            <div style={{display:"flex",gap:4,marginBottom:16}}>
              {[{id:"mine",label:"My Projects"},{id:"all",label:"All Projects"}].map(t=>(
                <button key={t.id} onClick={()=>setSubView(t.id)} style={{
                  padding:"8px 16px",background:subView===t.id?"#2563eb":"transparent",color:subView===t.id?"#fff":"var(--txt2)",
